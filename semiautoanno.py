...
class SemiAutoAnno:
    ...
    def fitTracking(self, LiInit=None, num_iter=50, useLagrange=False):
        ...
        print "joint length shape ", self.jointLength.shape
        #print "hc breaks shape ", self.msra.shape
        insert = numpy.insert(self.hcBreaks, 0, -1)
        print "insert shape ", insert.shape
        eddif = numpy.ediff1d(insert)
        print "ediff1d shape ", eddif.shape
        repeat = numpy.repeat(self.jointLength, eddif, axis=0)
        print "repeat shape ", repeat.shape
        num_square = numpy.square(repeat)
        print "shape ", num_square.shape
        eq = di - num_square
